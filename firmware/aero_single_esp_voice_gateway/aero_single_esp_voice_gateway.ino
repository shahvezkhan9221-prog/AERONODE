...
<div class="voice"><div class="voice-top"><span class="mic">●</span><div class="voice-copy"><b>Short voice note</b><small>Record about 3–5 seconds · maximum 45 KB</small></div></div><input hidden id="voiceFile" type="file" accept="audio/*" capture><button id="recordButton" class="record" onclick="document.getElementById('voiceFile').click()">🎙 Record or choose voice note</button><div id="voiceProgress" class="progress"><div class="track"><div id="voiceBar" class="bar"></div></div><div class="state"><span id="voiceState">Ready</span><span id="voicePercent">0%</span></div></div></div>
...
document.getElementById('voiceFile').addEventListener('change',async e=>{const file=e.target.files[0],button=document.getElementById('recordButton');if(!file)return;if(file.size>45000){setVoice('Voice note is too large — record a shorter clip',0);e.target.value='';return}button.disabled=true;const started=Date.now();try{setVoice('Encoding voice note…',12);const data=await readBase64(file);await wait(1500);setVoice('Encoded · preparing secure local transfer',28);const chunkSize=480,total=Math.ceil(data.length/chunkSize),voiceId='VOICE-'+Date.now().toString(36).toUpperCase();for(let i=0;i<total;i++){const response=await post('/voice',{voiceId,userId,name:survivorName,mime:file.type||'audio/mp4',index:String(i),total:String(total),chunk:data.slice(i*chunkSize,(i+1)*chunkSize)});if(!response.ok)throw new Error('Gateway rejected audio chunk');setVoice('Sending through Aero-Node gateway…',28+Math.round((i+1)/total*62))}await post('/voice-complete',{voiceId,userId,name:survivorName,mime:file.type||'audio/mp4',bytes:String(file.size),chunks:String(total)});const remaining=5000-(Date.now()-started);if(remaining>0)await wait(remaining);setVoice('✓ Voice note sent to rescue command',100);loadMessages()}catch(error){setVoice('Transfer failed — please try again',0)}finally{button.disabled=false;e.target.value=''}});
