  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      people.forEach((person) => {
        ...
        const confidence = person.locationKind === "approximate" ? "approximate" : "exact";
        const icon = L.divIcon({ className: "custom-marker-wrap", html: `<button class="bright-person-marker ${person.priority} ${confidence} ${person.id === selectedId ? "chosen" : ""}" aria-label="${safeName}"><i></i><span>${safeName}${confidence === "approximate" ? " · approx" : ""}</span></button>`, iconSize: [110, 52], iconAnchor: [55, 26] });
        L.marker(position, { icon }).on("click", () => onSelect(person.id)).bindTooltip(`${person.nodeId} · ${person.id} · ${confidence} location`, { direction: "top", offset: [0, -16] }).addTo(layerRef.current);
      });
